...
from ..utils.misc import norm_cdf, phi_0, phi_1, phi_2, sigmoid, leaky_relu
...
class LeakyReluChannel(Channel):
    ...
    def beliefs_measure(self, az, ax, tau, f):
        ...
        if ax==0 or u_eff==0:
            sx_eff_pos = np.sqrt(ax * (ax * tau + 1))
            sx_eff_neg = np.sqrt(ax * (self.slope**2 * ax * tau + 1))
            ...
            cov_neg = np.array([
                [ax * (self.slope**2 * ax * tau + 1), self.slope * ax * u_eff],
