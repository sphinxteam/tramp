...
class AMP_matrix_factorization(object):
    def __init__(self, K=1, N=1000, M=1000, Delta=5e-3, model='UV', var_mean = [1, 1 ,0, 0], seed=False, verbose=False, max_iter='inf'):
        # Parameters
        ...
        self.Delta = max(Delta, 1e-2)  # cut-off to avoid divergence
        np.random.seed(0) if seed else 0  # Seed
        self.verbose = verbose

        ## Variance and means
        self.lambda_X, self.Sigma_X = 0, 1
        Sigma_u, Sigma_v, lambda_u, lambda_v = var_mean
        self.lambda_v, self.Sigma_v = lambda_v * np.ones((self.K, self.N)), Sigma_v * np.identity(self.K)  # mean and covariance for v
        self.lambda_u, self.Sigma_u = lambda_u * np.ones((self.K, self.M)), Sigma_u * np.identity(self.K)  # mean and covariance for u

        # Convergence of the algorithm
        self.min_step_AMP = 1
        if max_iter == 'inf':
            self.max_step_AMP = 100
        else :
        ...
        # Initialization
        self.initialization_mode = 'random'  # random, planted, deterministic
    ...
    def generate_S_R(self):
        ...
        self.R = - 1 / Delta * np.ones((self.M, self.N)) + self.S_square
        self.S = self.S.T
        self.S_square = self.S_square.T
        self.R = self.R.T

    def generate_training_set(self):
        """
        - Generates teacher weights U, V
        - Generates Y and U
        - Generates S and R
        """
        self.generate_U_V()
    ...
    def initialization_storage(self):
        ...
        self.V_hat = np.zeros((self.K, self.N))
        self.C_hat_V = np.zeros((self.K, self.K, self.N))
        # Mean and variance Updates: B_V, A_V
        self.B_V = np.zeros((self.K, self.N))
        self.A_V = np.zeros((self.K, self.K, self.N))

        # Store Onsager terms
        self.V_hat_onsager = np.zeros((self.K, self.N))
    ...
    def compute_MSE(self):
        ...
        """
        #MSE_v, MSE_u = 1 + self.q_v - 2 *self.m_v  , 1 + self.q_u - 2 *self.m_u
        MSE_v = 1 - self.m_v
        MSE_u = 1 - self.m_u
    ...
    def compute_difference(self):
        ...
        """
        diff_weights = 1/self.N * np.sum([norm(self.V_hat[:, j] - self.V_hat_onsager[:, j]) for j in range(
            self.N)]) + 1/self.M * np.sum([norm(self.U_hat[:, mu] - self.U_hat_onsager[:, mu]) for mu in range(self.M)])
        if self.K == 1:
            diff_overlaps = max([np.abs(
                self.m_v-self.m_v_old), np.abs(self.m_u-self.m_u_old)])
            diff_bayes = max(
                np.abs(np.array([self.m_v-self.q_v, self.m_u-self.q_u])))
        else:
            diff_overlaps = max([np.abs(norm(self.m_v-self.m_v_old)), np.abs(
                norm(self.m_u-self.m_u_old))]) / (self.K**2)
            diff_bayes = max(np.abs(np.array(
                [norm(self.m_v-self.q_v), norm(self.m_u-self.q_u)]))) / (self.K**2)
        ...
        if self.verbose:
            print(f'Diff {self.diff_mode}: {diff_.item():.4e}')
    ...
    def AMP_training(self):
        ...
        if True : 
            print(f'm_u: {self.m_u.item():.3f} m_v: {self.m_v.item():.3f}')
            # print(f'q_u: {self.q_u.item():.3f} q_v: {self.q_v.item():.3f}')
            # print(
            #     f'MSE_u: {self.MSE_u.item() :.3f} MSE_v: {self.MSE_v.item() :.3f}')

        return self.U_hat, self.V_hat
    ...
    def VAMP_step(self):
        ...
        """
        self.initialization_storage()
        self.generate_VAMP_training_set()
        self.initialization()
...
class VAMP_matrix_factorization(object):
    def __init__(self, K=1, N=1000, M=1000, model='UV', Delta=1, au_av_bu_bv=[1, 1, np.zeros((1, 1000)),  np.zeros((1, 1000))], ax_bx=[1, np.ones((1000, 1000))],  seed=False, verbose=False, max_iter='inf', test_mode=False, initialization_mode='deterministic'):
        # Parameters
        self.model = model  # Model 'XX' or 'UV'
        self.K, self.N, self.M = K, N, M
        assert N != 0
        ...
        if max_iter == 'inf':
            ...
            self.max_step_AMP = max_iter
        self.diff_mode = 'overlap'  # 'overlap' or 'weight'
        self.threshold_error_overlap, self.threshold_error_weight, self.threshold_bayes = 1e-5, 1e-5, 5e-2
        # Initialization
    ...
    def compute_MSE(self):
        ...
        MSE_u_ = np.mean((self.U - self.U_hat)**2)
        MSE_v_ = np.mean((self.V - self.V_hat)**2)
